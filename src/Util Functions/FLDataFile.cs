...
namespace FLScanIE.Util_Functions
{
    public class FLDataFile
    {
        ...
        {
            public Setting(string fileName, string sectionName, string settingName, string desc, object[] values)
            {
            ...
            public Int32 Int32(int index) { checkBounds(index); return Convert.ToInt32(values[index]); }
            public string Str(int index) { checkBounds(index); return values[index].ToString(); }
            public string UniStr(int index) { checkBounds(index); return DecodeUnicodeHex(Str(index)); }
            public float Float(int index) { checkBounds(index); return Convert.ToSingle(values[index], System.Globalization.NumberFormatInfo.InvariantInfo); }
            public int NumValues() { return values.Length; }

            private void checkBounds(int index)
            {
                if(index < 0 || index >= _values.Length)
                    throw new ArgumentOutOfRangeException("index", string.Format("Value number {0} not found for [{1}]{2}, file: {3}:{4}", index, _sectionName, _settingName, _fileName, _lineNumber));
            }

            public override string ToString()
            {
                return string.Format("[{0}] {1} = {2}", sectionName, settingName, values.Length == 0 ? "" : JoinValues());
            }
        ...
        public class Section
        {
            ...
            /// <summary>
            /// Returns all values for the specified setting in this section.
            /// </summary>
            /// <param name="settingName">Key name.</param>
            public List<Setting> GetSettings(string settingName)
            {
        ...
        void LoadBinaryIni(byte[] buf, string fileName)
        {
            ...
            while (p < buf.Length && p < strTableOffset)
            {
                ...
                section.desc = fileName + ":0x" + p.ToString("x") + " '" + sectionName + "'";
                ...
                while (sectionNumEntries-- > 0)
                {
                    ...
                    Setting setting = new Setting(fileName, sectionName, settingName, desc, values);
                        ...
                        else
                        {
                            ...
                            if (strLine != "")
                            {
                                ...
                                if (multiFields)
                                {
                                    ...
                                    set = new Setting(fileName, sec.sectionName, settingName, desc, settingValues);
                                }
                                else
                                {
        ...
        public bool AddSetting(string sectionName, string settingName, object[] settingValues)
        {
            foreach (Section sec in sections)
            {
                if (sec.sectionName.ToLowerInvariant() == sectionName.ToLowerInvariant())
                {
                    ...
                    sec.settings.Add(new Setting(filePath, sec.sectionName, settingName, "unknown", settingValues));
                    return true;
                }
            }
            ...
            ns.settings.Add(new Setting(filePath, sectionName, settingName, "unknown", settingValues));
            sections.Add(ns);
            return true;
        }
